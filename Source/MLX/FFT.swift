...
public enum MLXFFT {
    private static let defaultNorm = MLX_FFT_NORM_BACKWARD
    ...
    public static func ifft(
        ...
        var result = mlx_array_new()
        mlx_fft_ifft(
            &result, array.ctx, (n ?? array.dim(axis)).int32, axis.int32, defaultNorm, stream.ctx
        )
        return MLXArray(result)
    }
    ...
    public static func irfft(
        ...
        var result = mlx_array_new()
        mlx_fft_irfft(&result, array.ctx, n.int32, axis.int32, defaultNorm, stream.ctx)
        return MLXArray(result)
    }
